...
namespace nrcore {
    ...
    void Socks5::connect(String addr, unsigned short port) {
        ...
        const char _req[] = {0x05, 0x01, 0x00};
        ByteArray req(_req, 3);
        if (addr_type == Address::DOMAIN) {
            ...
            char addr_t = 1;
            Address _addr(Address::ADDR, addr);
            addr_type = _addr.getType();
            if (addr_type == Address::IPV6)
                addr_t = 4;
            req.append(&addr_t, 1);
            req.append((void*)_addr.getAddr(), (addr_type == Address::IPV4 ? sizeof(struct in_addr) : sizeof(struct in6_addr)));
        }
